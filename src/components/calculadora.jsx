...
const Calculadora = () => {
    ...
    const escritura = (event) =>{
        ...
        if(data.operacion.includes('Error')){
            ...
            setData({...data, operacion: `${data.operacion}` + valor}); //...data es operador de propagacion, copia todas las propiedades del objeto data actual
        }
    }

    const eliminar = () =>{
        setData({...data, operacion: data.operacion.slice(0, data.operacion.length-1)}); //slice() extrae una parte del string, en este caso desde el principio hasta el penultimo (asi borra el ultimo caracter)
    }
